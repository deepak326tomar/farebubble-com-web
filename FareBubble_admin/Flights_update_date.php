...
$button='button_add';
$button_title = 'Submit';	
$self = 'index.php?page=Flights_update_date';
$action="Add";
$id	='';
$type_deals_id ='';
$from_date ='';
...
if($day=='today')
{
$self='index.php?page=Flights_update_date&day=today';	
...
{
echo $day;
$self='index.php?page=Flights_update_date&day=yesterday';		
...
$self='index.php?page=Flights_update_date&day=month';		
...
if((isset($_REQUEST['from_date']) && $_REQUEST['from_date']!=''))
	{
			...
			insert($conn,'tb_flight_update_date',$data);
...
if(isset($_POST['button_edit']) && ($_POST['button_edit']!=''))
{
	...
	update($conn,"tb_flight_update_date",$data,"where id = '$id'");
	$stat = '2';
...
/*********        DISPLAY VALUE WHEN WE EDIT ANY RECORD START     ****** **/
	if($action=='edit')
	{
		...
		$button_title = 'Update';
		$form_area = '';
		$manu = selectfetch($conn,"*","tb_flight_update_date","where id = '$id'");
...
/*********        DELETE SINGLE START     ****** **/
	if($action=='del')
	{
		...
		if($tot_dist <=0 )
		{
			...
			update($conn,"tb_flight_update_date",$data_delete,"where id=".$id);
			$stat="4";
		...
		if($val == 'Enabled')
		{
			$new_val = 'Disabled';
				$data = array('status'=>$new_val,
							  'update_date'=>$update_date,
							  'update_by'=>$_SESSION['user_id']);
				update($conn,"tb_flight_update_date
",$data,"WHERE id ='".$id."'");
				save_action_history($conn,"2",$id);
				$stat='2';
			}
...
/*********       MULTIPLE ENABLED AND DISABLED STATUS END       ****** **/
	...
	//$sqll = mysqli_query($conn,"select * From t_flight_deals where id!='' and delete_status='False' $d_query  order by id desc");	
	//$sqll = mysqli_query($conn,"select a.* ,b.hot_deal_no from  t_hot_deals a  inner join tb_hot_deal_no b on a.hot_deal_id=b.id where a.delete_status='False' order by a.id asc");

	
	$sqll = mysqli_query($conn,"select a.* ,b.destination_type from tb_flight_update_date a inner join tb_destination_type b on a.type_deals_id=b.id where a.delete_status='False' order by id desc");
				...
				<div class="row-fluid" id="abc" <?php echo $form_area; ?>>
					<div class="row-fluid">
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <input type="text" data-date-format="dd-mm-yyyy"  id="from_date" name="from_date" placeholder="Enter update date" value="<?php echo $from_date; ?>" class="m-wrap width100" />
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <input type="text" name="times" placeholder="Enter time"  value="<?php echo $times; ?>" class="m-wrap width100" />                              
                            </div>
                        </div>
						</div>
						<div class="span3">
                        <div class="control-group">
                            <label class="control-label">Shift Day/Night</label>
                            <div class="controls">
                                <select class="m-wrap" name="Shift" tabindex="1">
					...
					<div class="span12">
						...
						<div class="portlet box light-grey">
							<div class="portlet-title">
								<h4><i class="icon-globe"></i>Update Date flights Deals=<?=$tot_discount?></h4>
								<div class="tools">
