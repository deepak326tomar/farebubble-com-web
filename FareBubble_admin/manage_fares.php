...
$button='button_add';
$button_title = 'Submit';	
$self = 'index.php?page=manage_fares';
$action="Add";
$id	='';
$origin ='';
$Origin_code ='';
...
if($day=='today')
{
$self='index.php?page=manage_fares&day=today';	
...
{
echo $day;
$self='index.php?page=manage_fares&day=yesterday';		
...
$self='index.php?page=manage_fares&day=month';		
...
if((isset($_REQUEST['origin']) && $_REQUEST['origin']!=''))
	{
			...
			insert($conn,'tb_destination_fare',$data);
...
if(isset($_POST['button_edit']) && ($_POST['button_edit']!=''))
{
	...
	update($conn,"tb_destination_fare",$data,"where id = '$id'");
	$stat = '2';
...
/*********        DISPLAY VALUE WHEN WE EDIT ANY RECORD START     ****** **/
	if($action=='edit')
	{
		...
		$button_title = 'Update';
		$form_area = '';
		$manu = selectfetch($conn,"*","tb_destination_fare","where id = '$id'");
...
/*********        DELETE SINGLE START     ****** **/
	if($action=='del')
	{
		...
		if($tot_dist <=0 )
		{
			...
			update($conn,"tb_destination_fare",$data_delete,"where id=".$id);
			$stat="4";
		...
		if($val == 'Enabled')
		{
			$new_val = 'Disabled';
				$data = array('status'=>$new_val,
							  'update_date'=>$update_date,
							  'update_by'=>$_SESSION['user_id']);
				update($conn,"tb_destination_fare
",$data,"WHERE id ='".$id."'");
				save_action_history($conn,"2",$id);
				$stat='2';
			}
...
/*********       MULTIPLE ENABLED AND DISABLED STATUS END       ****** **/
	...
	//$sqll = mysqli_query($conn,"select a.* ,b.destination_type from t_flight_deals a inner join tb_destination_type b on a.type_deals_id=b.id where a.delete_status='False' and a.page_id=a.page_id");
   

	$sqll = mysqli_query($conn,"select * From tb_destination_fare where id!='' and delete_status='False' and page_id='$page_id'");
					...
					<div class="span12">
						...
						<div class="portlet box light-grey">
							<div class="portlet-title">
								<h4><i class="icon-globe"></i>Total fares=<?=$tot_discount?></h4>
								<div class="tools">
