	  ...
	  $id = $_REQUEST['id'];
		...
		if($id > 0 )
		{
			$data_delete = array('delete_date' =>$delete_date,
									'delete_status' => 'True');
			update($conn,"tb_enquiry",$data_delete,"where id=".$id);
			$stat="4";
	...
	if( $action=='status')
	{
		...
		if($val == 'Enabled')
		{
			$new_val = 'Disabled';
				...
				update($conn,"tb_enquiry",$data,"WHERE id ='".$id."'");
...
$tot_discount = totalrows($conn,"tb_enquiry","where status='Enabled' and delete_status='False'" );
//inner join lk_tb_enquiry_status c on c.product_status_id=b.product_status_id
	$sqll = mysqli_query($conn,"SELECT *  from  tb_enquiry where delete_status='False' ORDER BY id DESC");	
	...
	<!-- BEGIN CONTAINER -->	
		...
		<!-- BEGIN PAGE -->
			...
			<div class="container-fluid">
				...
				$excel_query="select * from tb_enquiry where delete_status='False'";						
										  if(isset($_POST['sent'])){
								...
								<table class="table table-striped table-bordered table-hover" id="sample_1">
									...
									?>
										<tr class="odd gradeX">
											...
											<td><?php echo $row['reservation_from']; ?></td>
											<td><?php echo $row['reservation_to']; ?></td>
