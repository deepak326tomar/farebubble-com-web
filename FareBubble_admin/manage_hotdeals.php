...
$button='button_add';
$button_title = 'Submit';	
$self = 'index.php?page=manage_hotdeals';
$action="Add"; 
$id	='';
$hot_deal_id ='';
$origin ='';
...
if($day=='today')
{
$self='index.php?page=manage_hotdeals&day=today';	
...
{
echo $day;
$self='index.php?page=manage_hotdeals&day=yesterday';		
...
$self='index.php?page=manage_hotdeals&day=month';		
...
if((isset($_REQUEST['hot_deal_id']) && $_REQUEST['hot_deal_id']!=''))
	{
...
$manu_docc = selectfetch($conn,"airline_image","t_hot_deals","where id = '".$id."' ");
...
$airline_image=$old_airline_image;
	...
	update($conn,"t_hot_deals",$data,"where id = '$id'");
	$stat = '2';
...
/*********        DISPLAY VALUE WHEN WE EDIT ANY RECORD START     ****** **/
	if($action=='edit')
	{
		...
		$button_title = 'Update';
		$form_area = '';
		$manu = selectfetch($conn,"*","t_hot_deals","where id = '$id'");
...
/*********        DELETE SINGLE START     ****** **/
	if($action=='del')
	{
		...
		if($tot_dist <=0 )
		{
			...
			update($conn,"t_hot_deals",$data_delete,"where id=".$id);
			$stat="4";
				...
				<div class="row-fluid" id="abc" <?php echo $form_area; ?>>
					<div class="row-fluid">
                          ...
                          <div class="control-group">
                            <label class="control-label">Hot Deals Type</label>
                            <div class="controls">
                                <select class="m-wrap" name="hot_deal_id" tabindex="1" required="required">
			                   <option value="">Select Hot Deals Type</option>
							  <?php $sco_pages=mysqli_query($conn,"select * from tb_hot_deal_no");

							  while($row_sco=mysqli_fetch_assoc($sco_pages))
							  { 
							  	?>
								 <option value="<?php echo $row_sco['id'];?>" <?php echo selected($row_sco['id'],$hot_deal_id);?>><?php echo $row_sco['hot_deal_no'];?></option>
							<?php } ?>
				           </select>                                
                            </div>
                        </div>
					...
					<div class="span12">
						...
						<div class="portlet box light-grey">
							<div class="portlet-title">
								<h4><i class="icon-globe"></i>Total Hot Deals=<?php echo $tot_discount;?></h4>
								<div class="tools">
