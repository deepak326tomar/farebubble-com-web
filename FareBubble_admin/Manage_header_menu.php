...
$button='button_add';
$button_title = 'Submit';	
$self = 'index.php?page=Manage_header_menu';
$action="Add"; 
$id	='';
$pages_id	='';
$short_code	='';
...
if($day=='today')
{
$self='index.php?page=Manage_header_menu&day=today';	
...
{
echo $day;
$self='index.php?page=Manage_header_menu&day=yesterday';		
...
$self='index.php?page=Manage_header_menu&day=month';		
...
$tot_comp = totalrows($conn,"tb_header_menu","WHERE  short_code ='".$short_code."' and delete_status='False' and pages_id='".$pages_id."'");
               if($tot_comp > 0)
                {
               $button='button_add';
               $stat = '6';
		...
		echo '<script type="text/javascript">window.location="'.$self.'&stat='.$stat.'";</script>';
			...
			insert($conn,'tb_header_menu',$data);
...
if(isset($_POST['button_edit']) && ($_POST['button_edit']!=''))
{
	...
	$tot_comp = totalrows($conn,"tb_header_menu","WHERE  short_code ='".trim(strtoupper($_POST['short_code']))."' and delete_status='False' and pages_id='".$pages_id."' and id!='$id'");
		if($tot_comp > 0)
		{
			$button='button_add';
			 $stat = '3';
	...
	update($conn,"tb_header_menu",$data,"where id = '$id'");
	$stat = '2';
...
/*********        DISPLAY VALUE WHEN WE EDIT ANY RECORD START     ****** **/
	if($action=='edit')
	{
		...
		$button_title = 'Update';
		$form_area = '';
		$manu = selectfetch($conn,"*","tb_header_menu","where id = '$id'");
...
/*********        DELETE SINGLE START     ****** **/
	if($action=='del')
	{
		...
		if($tot_dist <=0 )
		{
			...
			update($conn,"tb_header_menu",$data_delete,"where id=".$id);
			$stat="4";
...
/*********       MULTIPLE ENABLED AND DISABLED STATUS END       ****** **/
	...
	//$sqll = mysqli_query($conn,"select * From tb_header_menu where id!='' and delete_status='False' $d_query  order by id desc");	
	
	
	$sqll = mysqli_query($conn,"select a.* ,b.page_name	 from  tb_header_menu a  inner join tb_header_menu_name b on a.pages_id=b.id where a.delete_status='False' $d_query  order by a.id asc");
					...
					<div class="row-fluid">
                          ...
                          <div class="control-group">
                            ...
                            <div class="controls">
							  ...
							  <?php $sco_pages=mysqli_query($conn,"select * from tb_header_menu_name");

							  while($row_sco=mysqli_fetch_assoc($sco_pages))
							  { 
							  	?>
								 <option value="<?=$row_sco['id']?>" <?php echo selected($row_sco['id'],$pages_id);?>><?=$row_sco['page_name']?></option>
							<? } ?>
				           </select>                               
                            </div>
                        </div>
					...
					<div class="span12">
						...
						<div class="portlet box light-grey">
							<div class="portlet-title">
								<h4><i class="icon-globe"></i>Total Manage Header Menu=<?=$tot_discount?></h4>
								<div class="tools">
