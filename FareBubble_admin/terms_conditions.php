...
$button='button_add';
$button_title = 'Submit';	
$self = 'index.php?page=terms_conditions';
$action="Add";
$page_id	='';
 $title_name  ='';
 $sub_title  ='';
...
insert($conn,'tb_terms_conditions',$data);
...
$manu_docc = selectfetch($conn,"title_name","tb_privacy_policy","where id = '".$id."' ");
	...
	update($conn,"tb_terms_conditions",$data,"where id = '$id'");
	$stat = '2';
...
/*********        DISPLAY VALUE WHEN WE EDIT ANY RECORD START     ****** **/
	if($action=='edit')
	{
		...
		$button_title = 'Update';
		$form_area = '';
		$manu = selectfetch($conn,"*","tb_terms_conditions","where id = '$id'");
		$id		=	$manu->id;
...
/*********        DELETE SINGLE START     ****** **/
	if($action=='del')
	{
		...
		if($tot_dist <=0 )
		{
			...
			update($conn,"tb_terms_conditions",$data_delete,"where id=".$id);
			$stat="4";
		...
		if($val == 'Enabled')
		{
			$new_val = 'Disabled';
				$data = array('status'=>$new_val,
							  'update_date'=>$update_date,
							  'update_by'=>$_SESSION['user_id']);
				update($conn,"tb_terms_conditions
...
/*********       MULTIPLE ENABLED AND DISABLED STATUS END       ****** **/
	...
	$sqll = mysqli_query($conn,"select * From tb_terms_conditions where delete_status='False'  order by id desc");	
					...
					<div class="span12">
						...
						<div class="portlet box light-grey">
							<div class="portlet-title">
								<h4><i class="icon-globe"></i>Total Manage Terms Conditions  Details=<?=$tot_discount?></h4>
								<div class="tools">
