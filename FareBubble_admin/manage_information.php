...
$button='button_add';
$button_title = 'Submit';	
$self = 'index.php?page=manage_information';
$action="Add";
$id	='';
$pages_id	='';
 $title_name  ='';
...
insert($conn,'tb_manage_information',$data);
...
$manu_docc = selectfetch($conn,"title_name","tb_manage_information","where id = '".$id."' ");
	...
	update($conn,"tb_manage_information",$data,"where id = '$id'");
	$stat = '2';
...
/*********        DISPLAY VALUE WHEN WE EDIT ANY RECORD START     ****** **/
	if($action=='edit')
	{
		...
		$button_title = 'Update';
		$form_area = '';
		$manu = selectfetch($conn,"*","tb_manage_information","where id = '$id'");
		$id		=	$manu->id;
		$pages_id		=	$manu->pages_id;
		$title_name		=	$manu->title_name;
		$sub_title	=	$manu->sub_title; 
		$discription	=	$manu->discription;
		$status	=	$manu->status;
	}
...
/*********        DELETE SINGLE START     ****** **/
	if($action=='del')
	{
		...
		if($tot_dist <=0 )
		{
			...
			update($conn,"tb_manage_information",$data_delete,"where id=".$id);
			$stat="4";
		...
		if($val == 'Enabled')
		{
			$new_val = 'Disabled';
				$data = array('status'=>$new_val,
							  'update_date'=>$update_date,
							  'update_by'=>$_SESSION['user_id']);
				update($conn,"tb_manage_information
",$data,"WHERE id ='".$id."'");
...
/*********       MULTIPLE ENABLED AND DISABLED STATUS END       ****** **/
	...
	//$sqll = mysqli_query($conn,"select * From tb_manage_information where delete_status='False'  order by id desc");	
	
	$sqll = mysqli_query($conn,"select a.* ,b.page_name	 from  tb_manage_information a  inner join tb_information_page b on a.pages_id=b.id where a.delete_status='False' $d_query  order by a.id desc");
				...
				<div class="row-fluid" id="abc" <?php echo $form_area; ?>>
					<div class="row-fluid">
						<div class="span4">
                          <div class="control-group">
                            ...
                            <div class="controls">
							  ...
							  <?php $sco_pages=mysqli_query($conn,"select * from tb_information_page");
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls" style="width: 618px;">
       <textarea type="text" name="discription" placeholder="Description" class="jqte-test"><?php echo $discription;?></textarea>
				...
				<div class="row-fluid">
					...
					<h4><i class="icon-globe"></i>Total Manage About Us  Details=<?php echo $tot_discount;?></h4>
								<div class="tools">
