...
$button='button_add';
$button_title = 'Submit';	
$self = 'index.php?page=page_type_name';
$action="Add";
$page_type_id	='';
$page_type_name ='';
...
if((isset($_REQUEST['page_type_name']) && $_REQUEST['page_type_name']!=''))
	{
		...
		
		{
			...
			$data=array(
						'page_type_name' =>  trim($_POST['page_type_name']),
			...
			insert($conn,'lk_tb_page_type',$data);
...
{
	$page_type_id=$_REQUEST['page_type_id'];
	$update_date=date("Y-m-d H:i:s");

				
	$data=array(
						
						'page_type_name' =>  $_POST['page_type_name'],
	...
	update($conn,"lk_tb_page_type",$data,"where page_type_id = '$page_type_id'");
	$stat = '2';
	save_action_history($conn,"2",$page_type_id);
...
/*********        DISPLAY VALUE WHEN WE EDIT ANY RECORD START     ****** **/
	if($action=='edit')
	{
		...
		$button_title = 'Update';
		$form_area = '';
		$manu = selectfetch($conn,"*","lk_tb_page_type","where page_type_id = '$page_type_id'");
	
		$page_type_id		=	$manu->page_type_id;
		$page_type_name		=	$manu->page_type_name;
...
/*********        DELETE SINGLE START     ****** **/
	if($action=='del')
	{
		...
		if($tot_dist <=0 )
		{
			...
			update($conn,"lk_tb_page_type",$data_delete,"where page_type_id=".$page_type_id);
			$stat="4";
			save_action_history($conn,"3",$page_type_id);
		...
		if($val == 'Enabled')
		{
			$new_val = 'Disabled';
				$data = array('status'=>$new_val,
							  'update_date'=>$update_date,
							  'update_by'=>$_SESSION['user_id']);
				update($conn,"lk_tb_page_type
",$data,"WHERE page_type_id ='".$page_type_id."'");
				save_action_history($conn,"2",$page_type_id);
				$stat='2';
			}
		if($val == 'Disabled')
...
$sqll = mysqli_query($conn,"select * From lk_tb_page_type where page_type_id!='' and delete_status='False' $d_query  order by page_type_id desc");	
				...
				<div class="row-fluid" id="abc" <?php echo $form_area; ?>>
					...
					<div class="span4">
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <input type="text" name="page_type_name" placeholder="Enter page type name" value="<?php echo $page_type_name; ?>" width="245px"  class="large m-wrap" />
					...
					<div class="span12">
						...
						<div class="portlet box light-grey">
							<div class="portlet-title">
								<h4><i class="icon-globe"></i>Total page Name=<?=$tot_discount?></h4>
								<div class="tools">
								...
								<table class="table table-striped table-bordered table-hover" id="sample_1">
									...
									?>
										<tr class="odd gradeX">
                                            ...
                                            <td><?php echo $row['page_type_name']; ?></td>
                                           <td class="hidden-480"><?php echo $row['add_date']; ?></td>
											<td class="center hidden-480">
											<?php 
											if($row['add_by']=='0'){ echo 'Admin'; }else{ echo getvalue("page_type_name","lk_tb_page_type","where page_type_id='".$row['add_by']."'"); } 
											?>
											</td>
											
                                           <td>
											<a href="<?php echo  $self;?>&page_type_id=<?php echo $row['page_type_id']; ?>&val=<?php echo $row['status']; ?>&action=status"><img title="<?php echo  $row['status'];?>" src="assets/img/<?php  echo  $row['status'];?>.jpg" width="16" country="16" border="0" /></a>
											
                                            </td>
											<td >
												<a href="<?php echo  $self;?>&action=edit&page_type_id=<?php echo $row['page_type_id']; ?>"><img src="assets/img/pencil.jpg"/></a>
												&nbsp; 
												<a href="<?php echo  $self;?>&action=del&page_type_id=<?php echo $row['page_type_id']; ?>"><img src="assets/img/cross.jpg"/></a>
											</td>
