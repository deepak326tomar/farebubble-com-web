...
$button='button_add';
$button_title = 'Submit';	
$self = 'index.php?page=manage_airlines';
$action="Add";
$id	='';
$airline_name ='';
$airline_image='';
...
if((isset($_REQUEST['airline_name']) && $_REQUEST['airline_name']!=''))
	{
		$tot_comp = totalrows($conn,"t_airlines","WHERE  airline_name ='".trim(strtoupper($_POST['airline_name']))."' and delete_status='False'");
...
if(isset($_POST['button_edit']) && ($_POST['button_edit']!=''))
{
	...
	$tot_comp = totalrows($conn,"t_airlines","WHERE  airline_name ='".trim(strtoupper($_POST['airline_name']))."' and delete_status='False' and id!='$id'");
		if($tot_comp > 0)
		{
			$button='button_add';
			 $stat = '3';
		...
		{
$manu_docc = selectfetch($conn,"airline_image","t_airlines","where id = '".$id."' ");
...
$airline_image=$old_airline_image;
	...
	update($conn,"t_airlines",$data,"where id = '$id'");
	$stat = '2';
...
/*********        DISPLAY VALUE WHEN WE EDIT ANY RECORD START     ****** **/
	if($action=='edit')
	{
		...
		$button_title = 'Update';
		$form_area = '';
		$manu = selectfetch($conn,"*","t_airlines","where id = '$id'");
	
		$id		=	$manu->id;
		$airline_name		=	$manu->airline_name;
...
/*********        DELETE SINGLE START     ****** **/
	if($action=='del')
	{
		...
		if($tot_dist <=0 )
		{
			...
			update($conn,"t_airlines",$data_delete,"where id=".$id);
			$stat="4";
...
/*********       MULTIPLE ENABLED AND DISABLED STATUS END       ****** **/
	...
	$sqll = mysqli_query($conn,"select * From t_airlines where id!='' and delete_status='False'  order by id desc");	
				...
				<div class="row-fluid" id="abc" <?php echo $form_area; ?>>
					<div class="span4">
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <input type="text" name="airline_name" placeholder="Enter airline_name" width="245px" value="<?php echo $airline_name; ?>" class="large m-wrap" />
