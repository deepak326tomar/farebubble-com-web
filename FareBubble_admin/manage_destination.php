...
$button='button_add';
$button_title = 'Submit';	
$self = 'index.php?page=manage_destination';
$action="Add";
$page_id	='';
$short_code ='';
$destinations_type ='';
...
if($day=='today')
{
$self='index.php?page=manage_destination&day=today';	
...
{
echo $day;
$self='index.php?page=manage_destination&day=yesterday';		
...
$self='index.php?page=manage_destination&day=month';		
...
if(isset($_REQUEST['stat']))
{
	...
	if($stat==6)
	{
		$msg_lte = "Short Code Allready Exist!";
		$type = "warning";
		$font_style = 'fa-check';
	}
}
...
$data=array(
           ...
           'page_name' => str_replace(" ","-",strtolower($_POST['page_name'])),
...
//$page_og_image=mysqli_fetch_assoc(mysqli_query($conn,"select og_image from tb_pages where id='$page_id'"));
...
$og_image=$old_og_image;
		   ...
		   'og_description' => $_POST['og_description'],
           'og_url' => $_POST['og_url'],
		   'airport_title' => $_POST['airport_title'],
...
$hs_id=$arr_pr_hs[$key];
	...
	if($pr_name !='' && $h_code!='')
	{
		...
		update($conn,'tb_page_content',$data_pcode,"where id='$hs_id'");
		}
		else
		{
		$data_pcode=array(
...
/*********        DISPLAY VALUE WHEN WE EDIT ANY RECORD START     ****** **/
	if($action=='edit')
	{
		...
		$page_type_id		=	$manu->page_type_id;
...
/*********       MULTIPLE ENABLED AND DISABLED STATUS END       ****** **/
	...
	$sqll = mysqli_query($conn,"select * From tb_pages where page_id!='' and delete_status='False' $d_query   and page_type_id='3' order by page_id desc");	
		...
		<!-- BEGIN PAGE -->
			...
			<div class="container-fluid">
				...
				<div class="row-fluid" id="abc" <?php echo $form_area; ?>>
                   ...
                   $count=mysqli_query($conn,"select * from lk_tb_page_type where  page_type_id='3' limit 2");
	               ...
	               <option value="<?php echo $rows['page_type_id'];?>"><?php echo $rows['page_type_name'];?></option>
						...
						<div class="span4">
                        <div class="control-group">
                            <label class="control-label">Destinations Type </label>
                            <div class="controls">
                                <select class="m-wrap" name="destinations_type" tabindex="1">
								 <option value="Domestic" <?php echo selected($destinations_type,"Domestic");?>> Domestic Destinations</option>
								 <option value="International" <?php echo selected($destinations_type,"International");?>>International Destinations </option>
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <input type="text" name="page_display_name" placeholder=" Page display Name"     style="width:95%;" value="<?php echo $page_display_name; ?>" class="m-wrap" />
...
{  
	...
	?>
	<br>
<input type="text" name="paratitle[]" value="<?php echo $row_hs['paratitle']; ?>" class="add_inp" placeholder="Places to visit Title" style="margin: -20px 6px 5px 0;width:32%;">

<button onclick="delete_par(this.value)"  value="<?=$row_hs['id'];?>"  style="margin-bottom:24px;color: white;background: red;">Delete</button>
...
<script>
 ...
 <? }
				 ...
				 <div id="dynamicInput">
				  <br><input type="text" name="paratitle[]" class="add_inp" placeholder="Places to visit Title" style="margin: -20px 6px 5px 0;width:32%;">
					...
					<div class="span12">
						...
						<div class="portlet box light-grey">
							<div class="portlet-title">
								<h4><i class="icon-globe"></i>Total Manage Destination  Details=<?=$tot_discount?></h4>
								<div class="tools">
...
<td class="text-center"><a href="<?=$admin_path;?>index.php?page=manage_fares&page_id=<?php echo $row['page_id']; ?>" class="btn btn-outline-success">Add Fares</a></td>										  
...
function addInput(divName){
	...
	//var jqteStatus = true;
 	...
 	<script>
	function delete_paragraph(val)
                {
                    var postData={par_id:val};
                jQuery.ajax({
            type: "POST",
            url: 'ajax/delete_para.php',
            data: postData,
            success: function (result) {
           }
                          });   
...
function addInputs(divName){
     if (counters == limit)  {
          alert("You have reached the limit of adding " + counters + " input,textarea");
     }
     else {
          var newdiv = document.createElement('div');
          newdiv.innerHTML =  "<br><input type='text' name='best_time_title[]' class='add_inps' placeholder='questions' style='margin: -20px 6px 5px 0;width: 32%;' /> <Textarea type='text' name='best_time_content[]' placeholder='Answer' class='add_inps' large' style='margin: -2px 0 5px 0;width: 64.4%;' ></Textarea> ";
          document.getElementById(divName).appendChild(newdiv);
          counters++;
     }
}
